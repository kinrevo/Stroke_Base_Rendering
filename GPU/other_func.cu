...
//既存の2次元int配列を1次元int配列に変換
int* ReshapeInt_2to1(int **ally_2dim, int height, int width){

    int *ally_1dim = (int*)malloc(sizeof(int)*height*width);
    
    for(int i=0; i<height; i++) {
        for(int j=0; j<width; j++) {
            ally_1dim[i*width+j] = ally_2dim[j][i];
        }
    }
...
//既存の2次元double配列を1次元float配列に変換
float* ReshapeDouble_2to1(double **ally_2dim, int height, int width){

    float *ally_1dim = (float*)malloc(sizeof(float)*height*width);
...
double** gaussian_filter_d(double** in, int c, double** filter, int width, int height)
{
	...
	double sum=0;
	double fil_sum;
	double** new_ally = create_dally(width, height);
	//入力データをコピー
	copy_dally(in, new_ally, width, height);
